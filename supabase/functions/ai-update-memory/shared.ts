...
export async function regenerateMemory(studentId: string) {
  const db = svc()
  const [{ data: facts }, { data: chats }, { data: convSums }, { data: profile }, { data: userRow }] = await Promise.all([
...
Rebuild the memory from the CURRENT sources below. The sources are the complete truth: anything not in them does not exist (deleted documents must be forgotten). Rules:
- Facts must come only from the sources provided.
